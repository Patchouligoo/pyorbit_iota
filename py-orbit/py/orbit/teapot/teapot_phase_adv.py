...
class DriftTEAPOT(NodeTEAPOT):
	...
	def track(self, paramsDict):
		...
		global phase_adv_x
		phase_adv_x += length/self.twiss.getBeta(0)
		global phase_adv_y
		phase_adv_y += length/self.twiss.getBeta(1)
		global position
		position
		if length > 0 and rank == 0:
			phase_adv_writer = open("./phase_adv.txt", 'a')
			phase_adv_writer.write(str(self.getName()) + " " + str(index) + " " + str(self.getType()) + 
			" " + str(length) + " " + str(position) +  " " + str(length/self.twiss.getBeta(0)) +  " " + str(length/self.twiss.getBeta(1))
			+ " " + str(phase_adv_x) + " " + str(phase_adv_y) + "\n")
			phase_adv_writer.close()
		...
		self.twiss.analyzeBunch(bunch)
                ...
                for i in range(bunch.getSize()):
                        ...
                        if rank == 0 and (id < 0):
                                ...
                                f = open("/Users/Patchouli_goo/py-orbit/PROCEDURE_TEST/7_5_coasting_tests/tune_footprint_test/9e10_osc/test_data/test_" + str(int(-1 * id)) + ".txt", 'a')
