...
extern "C"
{
    ...
    static PyObject* wrap_dipedge(PyObject *self, PyObject *args)
    {
        ...
        double fint;
        double in_out;
        if(!PyArg_ParseTuple(	args, "Odddd|d:dipedge",
                             &pyBunch, &e1, &h, &hgap, &fint, &in_out))
        {
            error("teapotbase - dipedge - cannot parse arguments!");
        }
        Bunch* cpp_bunch = (Bunch*) ((pyORBIT_Object *) pyBunch)->cpp_obj;
        teapot_base::dipedge(cpp_bunch, e1, h, hgap, fint, in_out);
        Py_INCREF(Py_None);
        return Py_None;
    }

    static PyObject* wrap_nonlinearlens(PyObject *self, PyObject *args)
    {
        PyObject* pyBunch;
        double knll;
        double cnll;
        double L;
        double beta;
        if(!PyArg_ParseTuple(	args, "Odd:dipedge",
                             &pyBunch, &knll, &cnll))
        {
            error("teapotbase - dipedge - cannot parse arguments!");
        }
        Bunch* cpp_bunch = (Bunch*) ((pyORBIT_Object *) pyBunch)->cpp_obj;
        teapot_base::nonlinearlens(cpp_bunch, knll, cnll);
        Py_INCREF(Py_None);
        return Py_None;
    }
    ...
    static PyMethodDef teapotbaseMethods[] =
    {
            ...
            {"dipedge",          wrap_dipedge,        METH_VARARGS, "Tracking particles through a dipole edge."},
            {"nonlinearlens",    wrap_nonlinearlens,  METH_VARARGS, "Tracking particles through nonlinear lens"},
