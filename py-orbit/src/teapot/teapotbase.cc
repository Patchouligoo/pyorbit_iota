...
void dipedge(Bunch* bunch, double e1, double h, double hgap, double fint, double in_out)
{
    double g = 0;
    double psi = 0;
    
    g = 2.0 * hgap;
    psi = fint * g * h * (1.0 + sin(e1)*sin(e1))/cos(e1);

    double E = bunch->getSyncPart()->getEnergy();
    ...
    for(int i = 0; i < bunch->getSize(); i++)
    {
        ...
        double py = arr[i][3];
        double dE = arr[i][5];
        double delt = (dE/E)/(beta * beta);

        //arr[i][1] = arr[i][0] * -1.0 * tan(e1) * h + arr[i][1];

        if (in_out == -1) {
            arr[i][3] = y * -1.0 * tan(- psi) * h + py - h * y * py - h * psi * y * delt/(cos(e1 - psi) * cos(e1 - psi));
        }
        else if (in_out == 1) {
            arr[i][3] = y * -1.0 * tan(- psi) * h + py + h * y * py - h * psi * y * delt/(cos(e1 - psi) * cos(e1 - psi));
        }
        else {
            arr[i][3] = y * -1.0 * tan(- psi) * h + py;
        }
    }
...
void nonlinearlens(Bunch* bunch, double knll, double cnll)
{
    //cout << knll << " " << cnll << " " << L << "\n";
    double dd, u, v, dUu, dUv, dux, duy, dvx, dvy, dt, k, x, y, px, py;
    double pi = 3.141592653589;

    //double tau = knll/cnll;
   ...
   for(int ip = 0, nParts = bunch->getSize(); ip < nParts; ip++){
        ...
        complex<double> z(x/(cnll), y/(cnll));

        //cout << "\n\n" << x << " " << y << "\n";


        double tp = knll; //tau * pow(cnll, 2) / beta;

        complex<double> dFdz = tp/sqrt(1.0 - pow(z, 2)) * asin(z) + tp * pow(z, 2) / pow(1.0 - pow(z, 2), 1.5) * asin(z) + tp * z / (1.0 - pow(z, 2));

        //cout << z << " " << tp * pow(z, 2) / pow(1.0 - pow(z, 2), 1.5) * asin(z) << " " << tp/sqrt(1.0 - pow(z, 2)) * asin(z) << "\n";

        dFdz = dFdz/(cnll);

        double dpx = -1 * real(dFdz);
        double dpy = imag(dFdz);
