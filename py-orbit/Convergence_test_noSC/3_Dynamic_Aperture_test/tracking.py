...
for i in range(50):
    for j in range(100):
        b.addParticle(10*sx*float(j)/100 * cos(float(i)/50 * pi/2), 0, 10*sx*float(j)/100 * sin(float(i)/50 * pi/2), 0, 0, 0.000013229) #0.000013229

        b.partAttrValue("ParticleIdNumber", b.getSize() - 1, 0,  b.getSize() - 1)
